...
class ContactStorage: ContactStorageProtocol {
    ...
    func load() -> [ContactProtocol] {
        ...
        for contact in contactsFromStorage {
            guard let title = contact[ContactKey.title.rawValue], let phone = contact[ContactKey.title.rawValue] else {
                continue
            }
    ...
    func save(contacts: [ContactProtocol]) {
        ...
        contacts.forEach { contact in
            var newEementForStorage: Dictionary<String, String> = [:]
            newEementForStorage[ContactKey.title.rawValue] = contact.title
            newEementForStorage[ContactKey.title.rawValue] = contact.phone
            arrayForStorage.append(newEementForStorage)
